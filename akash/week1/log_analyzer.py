...
def analyze_logs(logs: list[str]) -> dict:
    ...
    for log in logs:
        ...
        status = int(temp[3].split("=", 1)[1])
        latency = int(temp[4].split("=", 1)[1])
        
        if endpoint not in log_dict:
            log_dict[endpoint] = [0, 0, 0]  # requests, errors, total latency
        log_dict[endpoint][0] += 1
        if status >= 400:
            log_dict[endpoint][1] += 1
        log_dict[endpoint][2] += latency
    ...
    for endpoint, (requests, errors, total_latency) in log_dict.items():
        error_rate_per_endpoint[endpoint] = round(errors / requests, 2) if requests > 0 else 0
        avg_latency_per_endpoint[endpoint] = round(total_latency / requests, 2) if requests > 0 else 0


    top_user = max(user_dict, key=user_dict.get)

    return {
        "total_requests": total_requests,
        "requests_per_endpoint": {endpoint: stats[0] for endpoint, stats in log_dict.items()},
